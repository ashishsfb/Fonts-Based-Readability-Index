...
<?php
	...
	if(!empty($_POST) && $_SERVER["REQUEST_METHOD"] == "POST"){
		...
		if(isset($_POST['signin']) and $_POST['signin']){
			...
			//0 new user with - either non existent table or non empty table
			if($count == 0){
				$sql = "SELECT 1 FROM main";
				$result = mysqli_query($connection, $sql);
				
				//no table exists with name main i.e. non existent
				if(empty($result)){
