...
<?php
    ...
    for($i = 0; $i <= 1; $i++){
		...
		while($row=mysqli_fetch_array($result)){
			...
			$view_count[$article_types[$i]]+=  mysqli_num_rows($result1);
			...
			<div class="tab-content">
				...
				<div class="tab-pane fade" id="ncert">
					...
					while($row=mysqli_fetch_array($result)){
						...
						$query1="SELECT * FROM test_data Where `pid`='$var1'";
						$result1=mysqli_query($connection, $query1);
						$totalncertviewers=  mysqli_num_rows($result1);
						$ncertmale=0;
						$ncertfemale=0;
									...
									echo "<div class='para_panel' id='ncert_para_panel".$i."'>
										...
										<div id='ncert_para".$i."' class='panel-collapse panel-body collapse'>";
											...
											for($j = 0; $j < 4; $j++){
												...
												$para=mysqli_query($connection, $query);
												$para_font_style_count[$j]=mysqli_num_rows($para);

												while($row_para=mysqli_fetch_array($para)){
													...
													$para1=mysqli_query($connection, $query);
													$row_para1=mysqli_fetch_array($para1);
											...
											for($j = 0; $j < 3; $j++){
												...
												$para=mysqli_query($connection, $query);
												$para_font_size_count[$j]=mysqli_num_rows($para);
											...
											for($j = 0; $j < 2; $j++){
												...
												$para=mysqli_query($connection, $query);
												$para_line_height_count[$j]=mysqli_num_rows($para);
											...
											for($j = 0; $j < 2; $j++){
												...
												$para=mysqli_query($connection, $query);
												$para_word_spacing_count[$j]=mysqli_num_rows($para);
				...
				<div class="tab-pane fade" id="wiki">
					...
					while($row=mysqli_fetch_array($result)){
						...
						$query1 = "SELECT * FROM test_data Where `pid`='$var1'";
						$result1 = mysqli_query($connection, $query1);
						$totalwikiviewers = mysqli_num_rows($result1);
						$wikimale = 0;
						$wikifemale = 0;
